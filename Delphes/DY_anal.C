...
#include "../../Generator/Delphes-3.4.2/classes/DelphesClasses.h" //-Edit path  Delphes_Directory_location/classes/DelphesClasses.h
...
int main() {
	...
	inChain->Add("/home/ico1036/Analysis/delphes/sample_data/DY.root"); //Read File

	TClonesArray* eTCA = new TClonesArray("Electron");
	inChain->SetBranchAddress("Electron",&eTCA); // Read Branch ( or leaf )

	TFile *outFile = new TFile("DYanal.root","recreate"); // output file format
